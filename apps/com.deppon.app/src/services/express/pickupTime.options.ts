import type {
  ...
  ExpressPickupTimeOpening,
  ExpressPickupTimeOpeningDate,
...
export interface ExpressPickupTimeSelection {
  date: string
  time: string
  text: string
...
export interface ExpressPickupTimeOption extends ExpressPickupTimeSelection {
  label: string
  disabled: boolean
  night: boolean
}

export interface ExpressPickupDateOption {
  value: string
  label: string
  weekday: string
  timeOptions: ExpressPickupTimeOption[]
}
...
function appendOpeningList(
  target: Map<string, Map<string, ExpressPickupTimeOpeningDate>>,
  openingList: ExpressPickupTimeOpening[] | undefined,
  type: ExpressPickupTimeOpeningType
) {
...
export function createExpressPickupDateOptions(
  response?: ExpressPickupTimeResponse | null
): ExpressPickupDateOption[] {
...
export function getFirstExpressPickupTimeSelection(
  response: ExpressPickupTimeResponse
...
export function createExpressPickupDateTime(date: string, time: string) {
...
export function findExpressPickupTimeSelection(
  response: ExpressPickupTimeResponse,
  ...
  for (const opening of response.openingList) {
    const item = opening.dateList.find(
      candidate =>
        ...
        createExpressPickupDateTime(opening.date, candidate.time) ===
          pickup.time
    )
